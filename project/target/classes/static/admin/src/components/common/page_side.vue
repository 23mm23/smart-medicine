    ...
    <el-menu-item index="/">
     ...
     <el-submenu index="user"
      ...
      <el-menu-item index="/user/table" v-show="user_group == '管理员'" @click="handleButtonClick(8)"><span>管理员</span></el-menu-item>
                                  <el-menu-item index="/physician_user/table" v-show="$check_action('/physician_user/table')" @click="handleButtonClick(8)">
                <span>医师用户</span></el-menu-item>
	    ...
	    <el-submenu class="Cust" :index="'child'+ item.id" v-for="(item) in i.children" :key="item.id" v-show="$check_action(item.path)">
          ...
          <template slot="title"><i class="el-icon-picture-outline"></i><span ref="customTemplate10">留言管理</span></template>
              <el-menu-item v-show="$check_action('/message/table')" index="/message/table" @click="handleButtonClick(10)">
                <span>{{$page_title("/message/table") || "用户反馈"}}</span>
              </el-menu-item>
        </el-submenu>
  ...
  export default {
    ...
    computed: {
      auth() {
        ...
        let site_list = []
		let map_list = []
                  		                    		                    		                                site_list.push("appointment_registration")
          		                                site_list.push("prescription_information")
          		                                site_list.push("admission_information")
          		                    		                    		                    		                  let list = ["exam", 'evaluation', "article", "article_type", "comment", "notice", "message"];
                                      list.push("physician_user");
