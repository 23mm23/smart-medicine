      ...
      <el-row class="rows row2">
         ...
         <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
                ...
                <el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
                                                      <el-button v-if="$check_action('/appointment_registration/table','del') || $check_action('/appointment_registration/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
                        </el-col>
      ...
      <el-table-column sortable prop="update_time" label="更新时间" min-width="200">
        <template slot-scope="scope">
          {{ $toTime(scope.row["update_time"],"yyyy-MM-dd hh:mm:ss") }}
                ...
                <el-table-column fixed="right" label="操作" min-width="120" v-if="$check_action('/appointment_registration/table','set') || $check_action('/appointment_registration/view','set') || $check_action('/appointment_registration/view','get')
												|| $check_action('/prescription_information/table','add') || $check_action('/prescription_information/view','add')
						|| $check_action('/admission_information/table','add') || $check_action('/admission_information/view','add')
											 ">
        

      <template slot-scope="scope">
        <router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
                     v-if="$check_action('/appointment_registration/table','set') || $check_action('/appointment_registration/view','set') || $check_action('/appointment_registration/view','get')"
                                 :to="'./view?' + field + '=' + scope.row[field]"
                                 size="small">
          <span>详情</span>
        </router-link>
                                        <!--跨表按钮-->
                  					                    <el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small"  @click="to_table(scope.row,'/prescription_information/view')" v-if="($check_action('/prescription_information/table','add') || $check_action('/prescription_information/view','add')) && !scope.row['prescription_information_limit']">
                      <span>开药</span>
                    </el-button>
					                  					                    <el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small"  @click="to_table(scope.row,'/admission_information/view')" v-if="($check_action('/admission_information/table','add') || $check_action('/admission_information/view','add')) && !scope.row['admission_information_limit']">
                      <span>入院</span>
                    </el-button>
					                                                                                  		  		        </template>
    </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        showModal: false,
        // 获取数据地址
        url_get_list: "~/api/appointment_registration/get_list?like=0",
        url_del: "~/api/appointment_registration/del?",
		  ...
		  get_list_after: function get_list_after(res, func, url) {
		  	let _this = this
		  		...
		  		let param = {
		  			...
		  			source_id: item.appointment_registration_id,
		  			source_user_id: _this.user.user_id
		  		};
		  		if(item.prescription_information_limit_times > 0){
		  			_this.$get("~/api/prescription_information/count?",param,(result)=>{
		  				if(result){
		  					if(result.result >= item.prescription_information_limit_times){
		  						_this.$set(item,'prescription_information_limit',true);
		  					}else{
		  						_this.$set(item,'prescription_information_limit',false);
		  					}
		  				}
		  			})
		  		}else{
		  		...
		  		if(item.admission_information_limit_times > 0){
		  			_this.$get("~/api/admission_information/count?",param,(result)=>{
		  				if(result){
		  					if(result.result >= item.admission_information_limit_times){
		  						_this.$set(item,'admission_information_limit',true);
		  					}else{
		  						_this.$set(item,'admission_information_limit',false);
		  					}
		  				}
		  			})
		  		}else{
